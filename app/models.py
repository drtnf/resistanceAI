...
class Game(db.Model):
    ...
    def start(self, students):
        ...
        #allocate spies
        self.spies_list = []
        while len(self.spies_list) < Agent.spy_count[self.num_players]:
            spy = random.randrange(self.num_players)
            if spy not in self.spies_list:
                self.spies_list.append(spy)
        self.spies_list.sort()
        self.spies = "".join([str(spy) for spy in self.spies_list])

        #start game for each agent
        for agent_id in range(self.num_players):
            spy_list = self.spies_list.copy() if agent_id in self.spies_list else []
            data = {
            ...
            print('new_game', data, students[agent_id])
...
#        return [p.agent_number for p in self.plays if p.student == student][0]
    ...
    def next_round(self, leader):
        ...
        if len(self.rounds) == 5: #game over
            spies_win = len([r for r in self.rounds if r.is_successful()]) < 3
            data = {
                    'game_id': self.game_id,
                    'spies_win': spies_win,
                    'spies': self.spies
